...
Return Value: -
******************************************************************************/
void send_stream(int fd, int input_number, int interval)
{
	...
	int m_frame_current;
	int m_frame_inc;
	int m_camera_frame_interval = pglobal->in[input_number].frame_interval;
	int sndsize = 128 * 1024;

	setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (char *) &sndsize,
			(int) sizeof(sndsize));

	if (interval && m_camera_frame_interval
			&& (m_camera_frame_interval < interval)) {
		m_frame_inc = PJS_FLOAT_EXT_CONST(m_camera_frame_interval) / interval;
	} else
...
void *client_thread(void *arg)
{
    ...
    if(strstr(buffer, "GET /?action=snapshot") != NULL) {
        ...
        input_suffixed = 255;
		req.type = A_STREAM;
		char *frpos = strstr(buffer, "&fr10=");
		if (frpos) {
			int framerate = atoi(frpos + 6);
			if (framerate > 0)
    ...
    case A_STREAM:
        ...
        send_stream(lcfd.fd, input_number,interval);
        break;
    case A_COMMAND:
