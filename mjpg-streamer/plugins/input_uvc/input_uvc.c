...
int input_init(input_parameter *param, int id)
{
    ...
    int width = 640, height = 480, fps = 5, fps_div =1, format = V4L2_PIX_FMT_MJPEG, i;
    ...
    param->global->in[id].frame_interval = 1000*fps_div/fps;

    /* display the parsed values */
    IPRINT("Using V4L2 device.: %s\n", dev);
    IPRINT("Desired Resolution: %i x %i\n", width, height);
    IPRINT("Frames Per Second.: %i/%i\n", fps,fps_div);
    ...
    if(init_videoIn(cams[id].videoIn, dev, width, height, fps, fps_div, format, 1, cams[id].pglobal, id) < 0) {
...
void help(void)
{
    ...
    fprintf(stderr, " [-f | --fps ]..........: frames per second\n" \
    		" [-s | --fpsdiv ]..........: frames per second divider\n" \
...
void *cam_thread(void *arg)
{
    ...
    while(!pglobal->stop) {
        ...
        if(pcontext->videoIn->fps < 60) {
            usleep(1000*1000*pcontext->videoIn->fps_div /pcontext->videoIn->fps - 5000);
        } else {
